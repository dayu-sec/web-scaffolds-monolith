...
- `pnpm test` 由 `apps/web` 的测试脚本驱动，用例位于 `apps/web/test/`；`apps/web/src/views/layout/` 等自治单元的用例与源码就近存放在 `__tests__/`。
- 用例断言可独立调用的模块行为、纯函数结果和数据文件一致性，不对源码文本、样式规则、目录结构或声明本身做正则断言；用例取舍与失败归属判定使用对应的前端测试工作流。
